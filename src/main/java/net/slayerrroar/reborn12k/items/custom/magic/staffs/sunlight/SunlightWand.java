package net.slayerrroar.reborn12k.items.custom.magic.staffs.sunlight;
...
public class SunlightWand extends Item {

    public SunlightWand(Settings settings) {
        super(settings);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity playerEntity, Hand hand) {
        ...
        playerEntity.addStatusEffect(new StatusEffectInstance(StatusEffects.REGENERATION, 20*10, 0, false, false));
        ...
        for (Entity e : world.getOtherEntities(playerEntity, Box.of(playerEntity.getPos(), 20, 20, 20))) {
            if (e instanceof PlayerEntity playerEntities) {
                if (playerEntity.distanceTo(playerEntities) < 20) {
                    playerEntities.addStatusEffect(new StatusEffectInstance(StatusEffects.REGENERATION, 20*10, 0, false, false));
                }
            }
        }
