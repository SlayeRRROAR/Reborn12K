...
public class SunlightStaff extends Item {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity playerEntity, Hand hand) {
        ...
        playerEntity.addStatusEffect(new StatusEffectInstance(StatusEffects.SATURATION, 20*60, 0, false, false));
        ...
        for(Entity e : world.getOtherEntities(playerEntity, Box.of(playerEntity.getPos(), 40, 40, 40))) {
            if(e instanceof PlayerEntity playerEntities) {
                if(playerEntity.distanceTo(playerEntities) < 40) {
                    playerEntities.addStatusEffect(new StatusEffectInstance(StatusEffects.SATURATION, 20*60, 0, false, false));
                }
            }
        }
