...
public class TerraRod extends Item {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity playerEntity, Hand hand) {
        ...
        playerEntity.addStatusEffect(new StatusEffectInstance(StatusEffects.REGENERATION, 20*10, 2, false, false));
        ...
        for (Entity e : world.getOtherEntities(playerEntity, Box.of(playerEntity.getPos(), 15, 15, 15))) {
            if (e instanceof LivingEntity livingEntity) {
                if (playerEntity.distanceTo(livingEntity) < 15) {
                    livingEntity.addStatusEffect(new StatusEffectInstance(StatusEffects.REGENERATION, 20*10, 2, false, false));
                }
            }
        }
