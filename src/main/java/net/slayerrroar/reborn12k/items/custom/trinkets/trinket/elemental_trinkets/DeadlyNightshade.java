...
public class DeadlyNightshade extends TrinketItem implements Trinket {
    ...
    public boolean canEquip(ItemStack stack, SlotReference slot, LivingEntity entity) {
        return TrinketsApi.getTrinketComponent(entity).get().isEquipped(TrinketItems.EARTH_PENDANT);
    }
    ...
    public void tick(ItemStack stack, SlotReference slot, LivingEntity entity) {
        ...
        if (!world.isClient) {
            if (!TrinketsApi.getTrinketComponent(entity).get().isEquipped(TrinketItems.EARTH_PENDANT)) {
                stack.decrement(1);
