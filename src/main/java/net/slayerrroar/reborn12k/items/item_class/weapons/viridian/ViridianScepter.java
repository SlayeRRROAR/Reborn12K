...
public class ViridianScepter extends Item {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity playerEntity, Hand hand) {
        ...
        playerEntity.addStatusEffect(new StatusEffectInstance(StatusEffects.ABSORPTION, 20*20, 2));
        playerEntity.addStatusEffect(new StatusEffectInstance(StatusEffects.RESISTANCE, 20*20, 1));
        ...
        for (Entity e : world.getOtherEntities(playerEntity, Box.of(playerEntity.getPos(), 30, 30, 30))) {
            if (e instanceof PlayerEntity playerEntities) {
                if (playerEntity.distanceTo(playerEntities) < 30) {
                    playerEntities.addStatusEffect(new StatusEffectInstance(StatusEffects.ABSORPTION, 20*20, 2));
                    playerEntities.addStatusEffect(new StatusEffectInstance(StatusEffects.RESISTANCE, 20*20, 1));
                }
            }
        }
