...
public class WindstormRod extends Item {

    public WindstormRod(Settings settings) {
        super(settings);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity playerEntity, Hand hand) {
        ...
        playerEntity.addStatusEffect(new StatusEffectInstance(StatusEffects.SPEED, 20*30, 2));
        playerEntity.addStatusEffect(new StatusEffectInstance(StatusEffects.HASTE, 20*30, 3));
        playerEntity.addStatusEffect(new StatusEffectInstance(StatusEffects.SATURATION, 20*30, 0));
        ...
        for (Entity e : world.getOtherEntities(playerEntity, Box.of(playerEntity.getPos(), 20, 20, 20))) {
            if (e instanceof PlayerEntity playerEntities) {
                if (playerEntity.distanceTo(playerEntities) < 20) {
                    playerEntities.addStatusEffect(new StatusEffectInstance(StatusEffects.SPEED, 20*30, 2));
                    playerEntities.addStatusEffect(new StatusEffectInstance(StatusEffects.HASTE, 20*30, 3));
                    playerEntities.addStatusEffect(new StatusEffectInstance(StatusEffects.SATURATION, 20*30, 0));
                }
            }
        }
