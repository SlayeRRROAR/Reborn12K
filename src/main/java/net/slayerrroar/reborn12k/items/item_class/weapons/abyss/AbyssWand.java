package net.slayerrroar.reborn12k.items.item_class.weapons.abyss;
...
public class AbyssWand extends Item {

    public AbyssWand(Settings settings) {
        super(settings);
    }
    ...
    public void appendTooltip(ItemStack itemStack, World world, List<Text> tooltip, TooltipContext tooltipContext) {
        ...
        tooltip.add(new TranslatableText("item.reborn12k.abyss.tooltip"));
    }
        ...
        for (Entity e : world.getOtherEntities(playerEntity, Box.of(playerEntity.getPos(), 25, 25, 25))) {
            if (e instanceof MobEntity mobEntity) {
                if (playerEntity.distanceTo(mobEntity) < 25) {
                    mobEntity.addStatusEffect(new StatusEffectInstance(StatusEffects.SLOWNESS, 20*20, 0));
                }
            }
        }
