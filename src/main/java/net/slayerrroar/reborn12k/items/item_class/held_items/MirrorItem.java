                    ...
                    if (serverPlayer.getSpawnPointPosition() != null)
                        {
                            ...
                            playerEntity.sendMessage(Text.translatable("item.reborn12k.magic_mirror.tooltip1"), true);   //tp to last bed
                            playerEntity.getItemCooldownManager().set(this, 20*3);
                            playerEntity.getStackInHand(hand).damage(1, world.random, null);
                        }
                    else
                    {
                        playerEntity.sendMessage(Text.translatable("item.reborn12k.magic_mirror.tooltip2"), true);  //bed not set
                        return new TypedActionResult<>(ActionResult.SUCCESS, playerEntity.getStackInHand(hand));
                    }
                }
                else
                {
                    playerEntity.sendMessage(Text.translatable("item.reborn12k.magic_mirror.tooltip3"), true);  //works only in the overworld
                }
            }
        }
        else
        {
            playerEntity.sendMessage(Text.translatable("item.reborn12k.magic_mirror.tooltip4"), true);  //no uses left
