...
public class ViridianStaff extends Item {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity playerEntity, Hand hand) {
        ...
        playerEntity.addStatusEffect(new StatusEffectInstance(StatusEffects.ABSORPTION, 20*30, 4));
        playerEntity.addStatusEffect(new StatusEffectInstance(StatusEffects.RESISTANCE, 20*30, 3));
        ...
        for (Entity e : world.getOtherEntities(playerEntity, Box.of(playerEntity.getPos(), 40, 40, 40))) {
            if (e instanceof PlayerEntity playerEntities) {
                if (playerEntity.distanceTo(playerEntities) < 40) {
                    playerEntities.addStatusEffect(new StatusEffectInstance(StatusEffects.ABSORPTION, 20*30, 4));
                    playerEntities.addStatusEffect(new StatusEffectInstance(StatusEffects.RESISTANCE, 20*30, 3));
                }
            }
        }
