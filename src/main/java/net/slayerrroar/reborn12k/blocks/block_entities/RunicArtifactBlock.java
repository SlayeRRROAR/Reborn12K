...
public class RunicArtifactBlock extends BlockWithEntity implements BlockEntityProvider {
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (state.getBlock() != newState.getBlock()) {
            ...
            if (blockEntity instanceof RunicArtifactBlockEntity) {
                ItemScatterer.spawn(world, pos, (RunicArtifactBlockEntity)blockEntity);
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new RunicArtifactBlockEntity(pos, state);
    }

    @Nullable
    @Override
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return checkType(type, CustomBlockEntities.RUNIC_ARTIFACT, RunicArtifactBlockEntity::tick);
    }
}
